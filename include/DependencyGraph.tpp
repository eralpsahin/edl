...
template <typename NodeT>
DependencyNode<NodeT> *DependencyGraph<NodeT>::getNodeByData(
    const NodeT *data) {
...
DependencyGraph<NodeT>::getNodeDepList(const NodeT *data) {
  DependencyNode<NodeT> *dNode = getNodeByData(data);
  return dNode->getDependencyList();
}

template <typename NodeT>
void DependencyGraph<NodeT>::addDependency(const NodeT *from, const NodeT *to,
...
template <typename NodeT>
DependencyType DependencyGraph<NodeT>::getDepType(const NodeT *data1,
                                                  const NodeT *data2) {
  try {
    DependencyType depType =
        DataToNodeMap[data1]->getDependencyType(DataToNodeMap[data2]);
    return depType;
  } catch (std::exception &e) {
    llvm::errs() << e.what() << "\n";
  }
