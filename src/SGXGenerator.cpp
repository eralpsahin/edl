...
static llvm::cl::opt<std::string> ecalls(
    "ecalls",
    llvm::cl::desc(
        "Specify the ECALL functions wrapper code path"),  // generated by edl
                                                           // pass
    llvm::cl::value_desc("ecalls"));
...
void pdg::SGXGenerator::generateAppFile(unsigned eidLine, unsigned initLine) {
  ...
  unsigned currLine = 1;
  unsigned ecallIdx = 0;
  std::ifstream ecallWrapper(ecalls);
  std::ofstream sgxApp("App.cpp");
  sgxApp << "#include \"Enclave_u.h\"\n#include \"sgx_urts.h\"\n#include "
            "\"sgx_utils.h\"\n";
  while (getline(appFile, line)) {
    if (currLine == eidLine) {
      sgxApp << ecallWrapper.rdbuf();
    } else if (currLine == initLine) {
      sgxApp << this->initialize << "\n";
    }
    int ecallIdx = searchECALL(currLine);
    if (ecallIdx != -1) {
      line.insert(ecallVec[ecallIdx].column - 1, "_ECALL");
      ecallIdx++;
    }
    sgxApp << line << "\n";
    currLine++;
...
int pdg::SGXGenerator::searchECALL(int line) {
  for (unsigned idx = 0; idx < ecallVec.size(); idx += 1) {
    if (ecallVec[idx].line == line) return idx;
  }
  return -1;
}
